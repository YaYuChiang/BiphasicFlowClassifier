...
class MLP(CNet):
    def __init__(self, input_dim, epochs=150, batch_size=16):
    ...
    def fit(self, data, label):
        ...
        for e in range(self.epochs):
            ...
            for iteration in bar:
                ...
                data_batch = torch.Tensor(data[index[iteration*self.batch_size:(iteration+1)*self.batch_size]])
                label_batch = torch.Tensor(label[index[iteration*self.batch_size:(iteration+1)*self.batch_size]])
    ...
    def predict(self, pred_data):
        ...
        for index in range(len(pred_data)):
            data_batch = torch.Tensor(pred_data[index:index+1])
            pred_batch = self.model(data_batch)
