...
def load_exp_csv(exp_num : int, path_train="none", path_test="none"):
    if exp_num not in [1, 2]:
        print(f"`{exp_num}` not a valid experiment")
        return None, None, None, None

    if not Path(path_train).is_file() or not Path(path_test).is_file():
        print(f"csv file `{path_train}` and/or `{path_test}` not exist(s)!")
        return None, None, None, None
